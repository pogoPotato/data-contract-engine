...
def test_cascade_delete_contract(test_db, sample_contract_data):
    ...
    version = ContractVersion(
        contract_id=contract.id, version="1.0.0", yaml_content="test"
    )
    result = ValidationResult(
        contract_id=contract.id, status="PASS", execution_time_ms=10.0
    )
    metric = QualityMetric(
        contract_id=contract.id, metric_date=date.today(), total_validations=10
    )
    ...
    assert (
        test_db.query(ContractVersion).filter_by(contract_id=contract.id).count() == 0
    )
    assert (
        test_db.query(ValidationResult).filter_by(contract_id=contract.id).count() == 0
    )
